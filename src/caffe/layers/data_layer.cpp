...
void* DataLayerPrefetch(void* layer_pointer) {
  ...
  const int mean_h_off  = (mean_height-height)/2;
  const int mean_w_off  = (mean_width-width)/2;
              ...
              top_data[((itemid * channels + c) * cropsize + h) * cropsize
                  ...
                  (static_cast<Dtype>(
                      (uint8_t)data[(c * height + h + h_off) * width
                                    + w + w_off])
                    - mean[(c * mean_height + mean_h_off + h + h_off) * mean_width
                           + mean_w_off + w + w_off])
                  * scale;
            }
          }
...
void DataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  caffe_modified::fake_leveldb* db_temp;
  std::string source_type = this->layer_param_.source_type();
  int max_buffered_files = 5;
  if ( source_type == "imagedb" ) {
	  max_buffered_files = 5;
	  db_temp = new caffe_modified::fake_leveldb_imagedb;
  } else if ( source_type == "bbox" ) {
	  max_buffered_files = 20;
	  db_temp = new caffe_modified::fake_leveldb_bbox;
  } else {
	  LOG(ERROR) << "Opening fake leveldb ( Type: "
			  << source_type << ") " << this->layer_param_.source();
  }

  std::string color_type_str = this->layer_param_.color_type();
  int color_type;
  if ( color_type_str == "color" ) {
	  ...
	  color_type = caffe_modified::fake_leveldb::AS_IS_COLOR ;
  } else {
  ...
  LOG(INFO) << "Opening fake leveldb ( Type: "
			  << source_type << ") " << this->layer_param_.source();
  int status = db_temp->OpenReadOnly( this->layer_param_.source() ) ;

  CHECK(~status) << "Failed to open fake leveldb "
      << this->layer_param_.source() << std::endl
      << "  Error code : " << status << std::endl;
  db_.reset(db_temp);
  iter_.reset(db_->NewIterator(max_buffered_files,
              this->layer_param_.canonical_size(), color_type) );
  iter_->SeekToFirst();
  ...
  Datum datum;
  std::pair<cv::Mat,int> ldata = iter_->get_labeled_image();
  ReadImageToDatum( ldata.first, ldata.second, 0,0, &datum );
  // image
