...
template <>
void caffe_copy<float>(const int N, const float* X, float* Y, const int incx, const int incy) {
  cblas_scopy(N, X, incx, Y, incy);
}

template <>
void caffe_copy<double>(const int N, const double* X, double* Y, const int incx, const int incy) {
  cblas_dcopy(N, X, incx, Y, incy);
}

template <>
void caffe_gpu_copy<float>(const int N, const float* X, float* Y, const int incx, const int incy) {
  CUBLAS_CHECK(cublasScopy(Caffe::cublas_handle(), N, X, incx, Y, incy));
}

template <>
void caffe_gpu_copy<double>(const int N, const double* X, double* Y, const int incx, const int incy) {
  CUBLAS_CHECK(cublasDcopy(Caffe::cublas_handle(), N, X, incx, Y, incy));
}
...
template <>
void caffe_cpu_tanh<float>(const int n, const float* x, float* y) {
  vsTanh( n, x, y );
}
...
template <>
void caffe_cpu_sech2<float>(const int n, const float* x, float* y) {
  vsCosh( n, x, y );
  vsInv( n, y, y );
  vsSqr( n, y, y );
}
...
template <>
void caffe_cpu_sech2<double>(const int n, const double* x, double* y) {
  vdCosh( n, x, y );
  vdInv( n, y, y );
  vdSqr( n, y, y );
}
...
template <>
double caffe_gpu_asum<double>(const int n, const double* x) {
	double y;
