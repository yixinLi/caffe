...
fake_leveldb_imagedb::Iterator* fake_leveldb_imagedb::NewIterator(
		size_t max_open_files, int canonical_size, int color_type ) {
...
struct compressed_image {
	...
	size_t length;
public:
	bool operator == ( const compressed_image& r_ ) const {
...
typedef std::vector< char > block_data;
...
public:
	...
	explicit image_loader_worker( int canonical_size, int color_type ) :
	...
	virtual cv::Mat operator () ( compressed_image c ) {
		...
		cv::Mat J;
		if ( canonical_size>0 ) {
			cv::resize( I, J, cv::Size(canonical_size,canonical_size) );
		} else {
			J = I;
		}
		return J;
...
struct _fake_leveldb_imagedb_iterator_inner {
	...
	_fake_leveldb_imagedb_iterator_inner( const std::string& root_dir,
			...
			size_t max_open_files, int canonical_size, int color_type ) :
				...
				iter(lines.begin()), max_open_files(max_open_files),
	...
	_fake_leveldb_imagedb_iterator_inner(
			const _fake_leveldb_imagedb_iterator_inner& _r ) :
				...
				iter(_r.iter), max_open_files(_r.max_open_files),
	...
	std::pair<cv::Mat,int> get_labeled_image() {
		...
		if ( current_filename != iter->first.filename ) {
			...
			while ( physical_file_list.size()>=max_open_files ) {
...
_fake_leveldb_imagedb_iterator::_fake_leveldb_imagedb_iterator( const std::string& root_dir,
		const fake_leveldb_imagedb::labeled_data_vec_t& lines,
		size_t max_open_files, int canonical_size, int color_type ) : _h( new
				_fake_leveldb_imagedb_iterator_inner
...
public:
	...
	bbox_cropper_worker( int canonical_size ) : canonical_size(canonical_size) {}
