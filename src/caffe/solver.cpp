...
template <typename Dtype>
void Solver<Dtype>::Solve(const char* resume_file, int resume_type ) {
  ...
  if (resume_file ) {
	  ...
	  case RESUME_FROM_CHECKPOINT:
		  ...
		  Restore(resume_file);
		  break;
	  case FINETUNE_PRETRAINED:
		  LOG(INFO) << "Load pre-trained model from " << resume_file;
		  net_->CopyTrainedLayersFrom( resume_file );
		  break;
	  default:
		  LOG(ERROR) << "Unrecognized resume_type";
	  }
