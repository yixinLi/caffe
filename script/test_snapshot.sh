...
TEST_NET_DIR=`readlink -f "$SCRIPT_DIR/../build/examples"`
...
ls $SNAPSHOT_PREFIX* | grep -v '\.solverstate$' | while read line; do
    ...
    mkfifo tmp_fifo
    cat tmp_fifo &
    GLOG_logtostderr=1 "$TEST_NET_DIR/test_net.bin" $DEVICE_ID \
        $VALIDATION_PROTOTXT $line 200 GPU 2>&1 | tee tmp_fifo | grep 'accuracy:' | grep 'Test accuracy:' | sed -e 's/^.*\] Test accuracy://' | tee  $fnonly.accuracy.doing
